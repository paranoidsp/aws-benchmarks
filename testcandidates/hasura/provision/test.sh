...
docker run --name graphql-engine -p 8080:8080  -d hasura/graphql-engine:dev-rtsopts-n-8406c7b graphql-engine +RTS -N -RTS --database-url "$(cat ~/postgres_credentials)" serve --cors-domain "https://localhost:9695" --server-port 8080 --enable-console -s $cpus -c 50
